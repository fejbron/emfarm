...
CREATE TABLE public.profiles (
  ...
  email TEXT NOT NULL,
  full_name TEXT,
  role TEXT NOT NULL CHECK (role IN ('manager', 'owner')) DEFAULT 'owner',
...
-- Profiles: Only managers can update other profiles (to change roles)
...
BEGIN
  ...
  IF user_count = 0 THEN
    assigned_role := 'manager'; -- First user gets manager role
  ELSE
...
-- 3. Protect main tables based on role (Manager = All, Owner = Select only)
